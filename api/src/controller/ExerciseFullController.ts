...
export class ExerciseFullController {
  ...
  static all = async (request: Request, response: Response, next: NextFunction) => {
    ...
    const result = getRepository(ExerciseFull).find({ relations: ["exerciseBase", 'workoutSession'] });
  ...
  static one = async (request: Request, response: Response, next: NextFunction) => {
    ...
    const result = await getRepository(ExerciseFull).findOne(request.params.id, { relations: ["exerciseBase", 'workoutSession'] });
    if (result instanceof Promise) {
      result.then(result => result !== null && result !== undefined ? response.send(result) : undefined);
  ...
  static allByWorkoutSession = async (request: Request, response: Response, next: NextFunction) => {
    ...
    const result = getRepository(ExerciseFull).find({ relations: ["exerciseBase", 'workoutSession'], where: { workoutSession: request.params.workoutSessionId } });
    if (result instanceof Promise) {
      result.then(result => result !== null && result !== undefined ? response.send(result) : undefined);
  ...
  static setDone = async (request: Request, response: Response, next: NextFunction) => {


    let { done, exerciseFullId, performance } = request.body;
    const exerciseFull = await getRepository(ExerciseFull).findOne(exerciseFullId, { relations: ["exerciseBase", 'workoutSession'] });


    exerciseFull.done = done
    exerciseFull.performance = performance
    const exerciseFullRepository = getRepository(ExerciseFull)
    try {
      await exerciseFullRepository.save(exerciseFull)
    } catch (e) {
    ...
    //If everything is fine, send 201 = CREATED
    response.status(201).send("ExerciseFull - Done completed");
  }
