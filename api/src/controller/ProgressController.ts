...
export class ProgressController {
  ...
  static allBySporter = async (request: Request, response: Response, next: NextFunction) => {

    const result = getRepository(Progress).find({ relations: ["sporter", "exerciseFull", "exerciseFull.exerciseBase"], where: { sporter: request.params.sporterId } });
    if (result instanceof Promise) {
      result.then(result => result !== null && result !== undefined ? response.send(result) : undefined);
  ...
  static save = async (request: Request, response: Response, next: NextFunction) => {
    ...
    let { sporterId, exerciseFullId, sets, reps, kg, time, performance } = request.body;
