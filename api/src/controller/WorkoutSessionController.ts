...
export class WorkoutSessionController {
  ...
  static allByFree = async (request: Request, response: Response, next: NextFunction) => {

    const result = getRepository(WorkoutSession).find({ relations: ["trainer"], where: { free: true } });
    if (result instanceof Promise) {
      result.then(result => result !== null && result !== undefined ? response.send(result) : undefined);
  ...
  static save = async (request: Request, response: Response, next: NextFunction) => {
    ...
    let { title, trainerId, type, muscleLevel, imageName, cardioLevel, exerciseBaseIds, free } = request.body;
  ...
  static update = async (request: Request, response: Response, next: NextFunction) => {
    ...
    let { workoutSessionId, title, trainerId, type, muscleLevel, imageName, cardioLevel, exerciseBaseIds, free } = request.body;
  ...
  static clone = async (request: Request, response: Response, next: NextFunction) => {
    ...
    //If everything is fine, send 201 = CREATED
    response.status(201).send({ id: workoutSession.id });
  }
  ...
  static setDone = async (request: Request, response: Response, next: NextFunction) => {
    ...
    let { workoutSessionId, done } = request.body;
    let workoutSession = await getRepository(WorkoutSession).findOne(workoutSessionId, { relations: ["trainer"] });

    workoutSession.done = done
